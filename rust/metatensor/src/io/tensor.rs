...
///
/// Pass `None` for any parameter to select all entries along that axis.
pub fn load_mmap_partial(
    ...
    let keys_raw = keys.map_or_else(
        crate::c_api::mts_labels_t::null,
        |l| l.as_mts_labels_t(),
    );
    let samples_raw = samples.map_or_else(
        crate::c_api::mts_labels_t::null,
        |l| l.as_mts_labels_t(),
    );
    let properties_raw = properties.map_or_else(
        crate::c_api::mts_labels_t::null,
        |l| l.as_mts_labels_t(),
    );

    let ptr = unsafe {
        crate::c_api::mts_tensormap_load_mmap_partial(
            path.as_ptr(),
            keys_raw,
            samples_raw,
            properties_raw,
