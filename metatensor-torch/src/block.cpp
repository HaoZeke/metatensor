...
Labels TensorBlockHolder::labels(uintptr_t axis) const {
    auto labels = torch::make_intrusive<LabelsHolder>(block_.labels(axis));
    // The Rust block's labels may be on a different device than the block
    // data (e.g., CPU-backed after to("meta") clones the Rust Labels).
    // Move labels to match data device for consistency.
