...
LabelsHolder::LabelsHolder(metatensor::Labels labels): labels_(std::move(labels)) {
    ...
    if (origin == TORCH_LABELS_ORIGIN && array.ptr != nullptr) {
        ...
        auto raw_labels = clone.as_mts_labels_t();

        // materialize CPU values if needed via mts_labels_values
        const int32_t* values_ptr = nullptr;
        size_t count = 0;
        metatensor::details::check_status(
            mts_labels_values(raw_labels, &values_ptr, &count)
        );
...
Labels LabelsHolder::to(torch::Device device, bool non_blocking) const {
    if (device == values_.device()) {
        ...
        // Meta tensors have no storage and cannot produce CPU values via
        // DLPack. Keep the original Rust Labels (which has CPU values from
        // the source) and only move the torch tensor to Meta.
        auto new_values = values_.to(device, non_blocking);
        return torch::make_intrusive<LabelsHolder>(
            this->names(),
            std::move(new_values),
            metatensor::Labels(this->as_metatensor())
        );
    } else {
        auto new_values = values_.to(device, non_blocking);
