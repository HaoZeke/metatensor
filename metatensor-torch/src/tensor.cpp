...
TensorMap TensorMapHolder::keys_to_properties(torch::IValue keys_to_move, bool sort_samples, double fill_value) const {
    ...
    // Ownership is transferred to the C API (Rust Drop handles cleanup).
    auto fv_mts = mts_array_t{};
    std::memset(&fv_mts, 0, sizeof(mts_array_t));
    if (this->keys()->count() > 0) {
        ...
        fv_mts = metatensor::DataArrayBase::to_mts_array_t(
