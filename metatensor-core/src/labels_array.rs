...
pub(crate) fn materialize_values_from_array(array: &mts_array_t, size: usize) -> Vec<LabelValue> {
    ...
    // LabelValue is #[repr(transparent)] over i32, so this is safe
    let label_values: Vec<LabelValue> = slice.iter().map(|&v| LabelValue::new(v)).collect();
...
unsafe extern "C" fn labels_array_as_dlpack(
    ...
    // Copy values as i32 (LabelValue is repr(transparent) over i32)
    let values: Vec<i32> = array_ref.values.iter().map(|v| v.i32()).collect();
